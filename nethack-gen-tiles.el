...
(defconst nethack-gen-tiles-files
  (mapcar (lambda (f) (file-name-concat "win" "share" f))
...
(defconst nethack-gen-tiles-grayscale-alist
  (let ((palette-labels)
    ...
    (range-map (lambda (c) (push c palette-labels))
               '(?. (?A . ?Z) (?0 . ?9)))
    (setq palette-labels (nreverse palette-labels))
...
(defun nethack-gen-tiles-read-glyph ()
  (when (search-forward-regexp "#.*\n{\n\\(\\(?:.*\n\\)+?\\)}" nil t)
    (mapcar #'string-trim (string-lines (match-string 1)))))
    ...
    (with-temp-file filename
      (insert (mapconcat #'identity
                         `(";;; This file in auto-generated with nethack-gen-tiles.el\n"
                           ...
                           ,@(mapcan (lambda (f) (nethack-gen-tiles-glyphs (file-name-concat dir f))) nethack-gen-tiles-files)
                           ;; monsters.txt is included twice, once in full color and again in grayscale
                           ,@(nethack-gen-tiles-glyphs (file-name-concat dir (car nethack-gen-tiles-files)) t)
                           ,(format "))\n\n(provide '%s-tiles)\n" variant))
                         "\n")))
